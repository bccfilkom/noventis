...
plt.rcParams.update({
    ...
    'figure.titlesize': 14, 'legend.fontsize': 10, 'figure.facecolor': '#010409', 'axes.facecolor': '#161B22', 
...
class EDAAnalyzer:
    ...
    def _generate_overview(self) -> str:
        return f"""<div class="grid-container"><div class="grid-item"><h4>Dataset Shape</h4><p><b>Rows:</b> {self.df.shape[0]}</p><p><b>Columns:</b> {self.df.shape[1]}</p></div><div class="grid-item"><h4>Column Types</h4><p><b>Numeric:</b> {len(self.numeric_cols_)}</p><p><b>Categorical:</b> {len(self.categorical_cols_)}</p></div><div class="grid-item"><h4>Memory Usage</h4><p>{(self.df.memory_usage(deep=True).sum() / 1024**2):.2f} MB</p></div></div><h3>Data Preview (First 5 Rows)</h3><div class='table-scroll-wrapper'>{self.df.head().to_html(classes='styled-table')}</div>"""
    ...
    def _analyze_outliers(self) -> str:
        ...
        for col in self.numeric_cols_:
            ...
            if not outliers.empty:
                outlier_found = True; fig, ax = plt.subplots(figsize=(8, 5)); sns.boxplot(x=self.df[col], ax=ax)
    ...
    def _analyze_numerical_distributions(self) -> str:
        ...
        all_panels_html = ""
        for col in self.numeric_cols_:
            skew_val = self.df[col].skew(); skew_label, label_class = ("Skewed", "label-skewed") if abs(skew_val) > 0.5 else ("Normal", "label-normal")
            ...
            all_panels_html += f"""<div class="panel-container"><div class="panel-title"><h3>{col}</h3></div><div class="panel-plot"><img src="{plot_b64}"></div>{info_html}</div>"""
        return all_panels_html
    def _plot_correlation_report(self) -> tuple[str, str]:
        ...
        dropdown_html = """<div class="filter-container"><label for="corr-filter">Highlight correlations (absolute value):</label><select id="corr-filter" onchange="filterCorrelationTable(this.value)"><option value="0.0">Show All</option><option value="0.5">&gt; 0.5</option><option value="0.7">&gt; 0.7</option></select></div>"""
        if len(self.numeric_cols_) > self.CORRELATION_COL_THRESHOLD:
            table_html = "<table class='styled-table sticky-index-table'><thead><tr><th></th>"
        ...
        all_summary_tables_html += f'<div id="corr-summary-0.7" class="corr-summary-table" style="display: none;"><h3>Top Positive Correlations (> 0.7)</h3>{pos_corr_html_07}<h3>Top Negative Correlations (< -0.7)</h3>{neg_corr_html_07}</div>'
        ...
        all_summary_tables_html += f'<div id="corr-summary-0.5" class="corr-summary-table" style="display: none;"><h3>Top Positive Correlations (> 0.5)</h3>{pos_corr_html_05}<h3>Top Negative Correlations (< -0.5)</h3>{neg_corr_html_05}</div>'
        return correlation_main_content_html, all_summary_tables_html
    ...
    def generate_html_report(self, report_height: int) -> HTML:
        """Assembles all generated content into a final interactive HTML report."""
        overview_html = self._generate_overview(); stats_html = self._generate_descriptive_stats()
        missing_summary_html, missing_plot_b64 = self._analyze_missing_values(); outliers_html = self._analyze_outliers()
        num_dist_html = self._analyze_numerical_distributions()
        correlation_main_content_html, correlation_summaries_html = self._plot_correlation_report()
        ...
        correlation_html = f"{correlation_main_content_html}{correlation_summaries_html}"
        
        html_template = f"""
        <!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Automated EDA Report</title>
        <style>
            ...
            .report-frame {{ height: {report_height}px; width: 100%; border: 1px solid var(--border-color); border-radius: 10px; overflow: hidden; }}
            .container {{ width: 100%; max-width: 1400px; margin: auto; background-color: var(--bg-dark-1); height: 100%; overflow: auto; }}
            ...
            .styled-table th, .styled-table td {{ border-bottom: 1px solid var(--border-color); padding: 0.8rem 1rem; text-align: left; white-space: nowrap; transition: opacity 0.3s ease, background-color 0.3s ease; }}
            ...
            .label-skewed {{ background-color: #D73A49; }} .label-normal {{ background-color: #28A745; }}
            .filter-container {{ margin: 1rem 0; }}
            .filter-container label {{ margin-right: 0.5rem; color: var(--text-muted); }}
            .filter-container select {{ background-color: var(--bg-dark-2); color: var(--text-light); border: 1px solid var(--border-color); border-radius: 5px; padding: 0.3rem; }}
        </style>
        </head><body><div class="report-frame"><div class="container">
            <header><h1>Automated EDA Report</h1><p>A comprehensive overview of the dataset's characteristics.</p></header>
        ...
        <script>
            ...
            function filterCorrelationTable(threshold) {{
                const thresholdNum = parseFloat(threshold);
                const table = document.querySelector('.sticky-index-table');
                if (table) {{
                    const cells = table.getElementsByTagName('td');
                    for (let i = 0; i < cells.length; i++) {{
                        const cell = cells[i];
                        const corrValStr = cell.getAttribute('data-corr');
                        if (corrValStr === null) continue;
                        const corrVal = parseFloat(corrValStr);
                        if (corrVal === 1.0) {{
                            cell.style.backgroundColor = 'var(--bg-dark-3)'; cell.style.fontWeight = 'bold'; cell.style.opacity = '1'; continue;
                        }}
                        if (Math.abs(corrVal) >= thresholdNum) {{
                            cell.style.opacity = '1';
                            if (thresholdNum > 0) {{
                               cell.style.backgroundColor = corrVal > 0 ? 'rgba(88, 166, 255, 0.2)' : 'rgba(247, 129, 102, 0.2)';
                            }} else {{ cell.style.backgroundColor = 'transparent'; }}
                        }} else {{
                            cell.style.opacity = '0.3'; cell.style.backgroundColor = 'transparent';
                        }}
                    }}
                }}
                document.querySelectorAll('.corr-summary-table').forEach(tableDiv => {{ tableDiv.style.display = 'none'; }});
                let idToShow = thresholdNum >= 0.7 ? 'corr-summary-0.7' : (thresholdNum >= 0.5 ? 'corr-summary-0.5' : 'corr-summary-0.7');
                const tableToShow = document.getElementById(idToShow);
                if (tableToShow) tableToShow.style.display = 'block';
            }}
            document.addEventListener('DOMContentLoaded', (event) => {{
                if (document.querySelector('.sticky-index-table')) {{
                    filterCorrelationTable(0.0);
                }}
            }});
        </script>
        </body></html>
    ...
    def run(self, report_height: int = 700) -> HTML:
        """
